    ...
    $.extend($.fn, {
        miniColors: function (o, data) {
            var create = function (input, o, data) {
                ...
                if (o.readonly) input.prop('readonly', true); if (o.disabled) disable(input);
                trigger.bind('click.miniColors', function (event) { event.preventDefault(); if (input.val() === '') input.val('#'); show(input) });
                input.bind('focus.miniColors', function (event) { if (input.val() === '') input.val('#'); show(input) });
                input.bind('blur.miniColors', function (event) {
                    var hex = expandHex(input.val());
                    input.val(hex ? '#' + convertCase(hex, input.data('letterCase')) : '')
                });
                input.bind('keydown.miniColors', function (event) { if (event.keyCode === 9) hide(input) });
                input.bind('keyup.miniColors', function (event) { setColorFromInput(input) });
                input.bind('paste.miniColors', function (event) { setTimeout(function () { setColorFromInput(input) }, 5) })
            };
            ...
            var enable = function (input) { input.prop('disabled', false).data('trigger').css('opacity', 1) };
            ...
            var show = function (input) {
                ...
                var hsb = input.data('hsb'); selector.find('.miniColors-colors').css('backgroundColor', '#' + hsb2hex({ h: hsb.h, s: 100, b: 100 }));
                ...
                $(document).bind('mousedown.miniColors touchstart.miniColors', function (event) {
                    ...
                    if ($(event.target).parents().andSelf().hasClass('miniColors-colors')) {
                        event.preventDefault(); input.data('moving', 'colors');
            ...
            var moveColor = function (input, event) {
                ...
                colorPicker.hide(); var position = { x: event.pageX, y: event.pageY };
            ...
            var moveHue = function (input, event) {
                ...
                var h = Math.round((150 - position.y - 1) * 2.4); if (h < 0) h = 0; if (h > 360) h = 360; var hsb = input.data('hsb');
                hsb.h = h; setColor(input, hsb, true)
            };
            ...
            var setColorFromInput = function (input) {
                ...
                setColor(input, hsb); return true
            };
            ...
            var getColorPositionFromHSB = function (hsb) {
                var x = Math.ceil(hsb.s / 0.67); if (x < 0) x = 0; if (x > 150) x = 150;
            ...
            var rgb2hsb = function (rgb) {
                ...
                if (hsb.s !== 0) {
                    ...
                    hsb.h = -1
                } hsb.h *= 60; if (hsb.h < 0) { hsb.h += 360 } hsb.s *= 100 / 255; hsb.b *= 100 / 255; return hsb
            };
            ...
            switch (o) {
                ...
                case 'value':
                    ...
                    $(this).each(function () {
                        if (!$(this).hasClass('miniColors')) return;
                        $(this).val(data); setColorFromInput($(this))
                    });
                    return $(this);
                case 'destroy':
                    $(this).each(function () {
                        if (!$(this).hasClass('miniColors')) return; destroy($(this))
                    });
                    return $(this);
                default: if (!o) o = {};
                    $(this).each(function () {
